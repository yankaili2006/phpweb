...
(function($){
	$.fn.getContent = function(downpageid){
		...
		$.ajax({
			type: "POST",
			url:"post.php",
			data: "act=getcontent&downpageid="+downpageid+"&nowid="+nowid,
			success: function(msg){
		...
		if($("#downpagesid")[0].value=="0"){
			...
			$('#downForm').ajaxSubmit({
				...
				success: function(msg) {
					if(msg=="OK"){
						$('div#notice').hide();
						$().alertwindow("下载修改成功","down_con.php");
...
$(document).ready(function(){
	...
	$('#downAddForm').submit(function(){

		SelectAll('spe_selec[]', 'select[]');

		$('#downAddForm').ajaxSubmit({
			target: 'div#notice',
			url: 'post.php',
			success: function(msg) {
				if(msg=="OK"){
					$('div#notice').hide();
					$().alertwindow("下载发布成功","down_con.php");
				...
				$.ajax({
					...
					success: function(msg){
						if(msg=="OK"){
							...
							var str="<a href='"+url+"' target='_blank'>http://.../down/class/?"+catid+".html</a>";
